...
\begin{abstract}
Stars in binary systems with small separations can interact through mass transfer; therefore, stars in binary or multiple systems may follow very different evolutionary pathways than single stars.
TYC 2597-735-1 is a candidate for a recent stellar merger remnant where a low-mass companion merged with the primary of the system a few thousand years ago. This violent event is evident in a conical outflow, now fading into intervening space, emitting exclusively in UV light and surrounding by leading shock filaments observed in H$\alpha$ and UV emission. From archival Chandra data, we identify X-ray emission from TYC 2597-735-1 itself with $\log(L_\mathrm{X}/L_\mathrm{bol})=-5.3$. A TESS lightcurve confirms a period of about 14~days. Together, this indicates ongoing stellar activity and the presence of magnetic fields on TYC 2597-735-1. Using stellar evolution models, we interpret the stellar activity as a sign of a newly formed convection zone in the stellar atmosphere that could lead to a convective dynamo. We also find X-ray emission from two small regions close to the shock fronts in the outflow. Either clumping in the ISM or an inhomogeneous mass and velocity distribution in the outflow could explain the localized X-ray emission in the outflow.
...
The BRN is seen brightly in the UV as a diffuse ring and, at further distance from TYC 2597-735-1, narrow H$\alpha$ filaments. \cite{2020Natur.587..387H} explain the H$\alpha$ as the forward shock of the outflow, caused by the ejecta driving a shock wave into the ISM, while the UV emission should be dominated by molecular H$_2$ emission, that is excited by a reverse shock. This reverse shock is caused by the deceleration of the outflow with the ISM and now travels backwards through the outflow.
...
Given the low number of counts and the size of the Chandra PSF that far away from the aimpoint, all three sources are compatible with point sources, but we expect that the latter two sources are probably spatially extended, since the H$\alpha$ and the UV emission in the region is also extended. However, the low count number does not allow us to actually fit the source shape or extent, so we treat them like point sources below.
...
We also estimated the background flux from the target pixel files (TPFs) via the tools \texttt{tesscut} \citep{Brasseur2019} and \texttt{eleanor} \citep{Feinstein2019}.
...
\cite{2020Natur.587..387H} noticed an RMS scatter around 0.008~mag --significantly larger than the formal uncertainties-- between runs of ground-based $B$ band observations between 2015 and 2019. They attributed this to systematic calibration uncertainties. However, given the variability in the TESS lightcurve on a similar level, this might instead be a signature of stellar activity cycles or flaring.
...
If we assume that a fraction $\eta_{\rm j} \sim 10\%$ of the accreted matter is launched into a bipolar collimated outflow (``jet'') with an outflow velocity $v_{\rm j}$ some multiple $\sim 2$ of the surface escape speed of TYC, $\approx (2GM_{\star}/R_{\star})^{1/2} \approx 230$ km s$^{-1}$, then the present-day jet power is given by
\be
...
\label{eqn:jetpower}
\ee
This is in principle sufficient to explain the X-ray point source overlapping TYC, $L_{\rm X} \sim 10^{31}$ erg s$^{-1}$. To match the temperature of the X-rays, we need internal shock speeds of order 500~km~s$^{-1}$ (eqn.~\ref{eqn:Tshock}).
...
The Rossby number, which is the ratio of the rotation period $P_{\rm rot}$ convective overturn time $\tau_{\rm conv}$, i.e.
\begin{equation}
...
\tablecolumns{7}
\tablehead{
	\colhead{Time} & \colhead{$R_{\star}$} & \colhead{$r_{\rm conv}$} & \colhead{$v_{\rm conv, max}$} & \colhead{$\tau_{\rm conv} = r_{\rm conv}/v_{\rm conv, max}$} & \colhead{{\bf Ro}} & \colhead{${\rm C_X}$} & \colhead{$\log(L_{\rm X} / L_{\rm bol})$} \\
...
The discussion above shows that the X-ray emission could be explained by either a very fast outflow component or coronal activity. In either case, magnetic fields need to be present on TYC 2597 735 1 to either accelerate the outflow or to power the corona, so the detection of X-ray emission provides strong circumstantial evidence of magnetic fields. Given that similar $\log(L_\mathrm{X}/L_\mathrm{bol})$ values are observed for comparable sources without accretion disks and outflows (Fig.~\ref{fig:lxbol}), it seem more likely that that emission can in fact be attributed to coronal activity. Deeper observations might be able to distinguish these possibilities: Coronal activity should be time-variable on scales of hours or days, while emission from a jot collimation shock is generated on much larger scales and varies more slowly. Also, longer observations might be able to resolve the spectrum into more temperature components; a corona might show hot plasma out of reach of a jet shock.
...
\ee
however depending sensitively on $kT_{\rm sh}$ (and hence $v_{\rm sh}$).  Thus, given a limit 
of few times $10^{31}$~erg~s$^{-1}$~pc$^{-2}$ (section~\ref{sec:xraydetection})
we can put an upper limit $n = n_{\rm max} < 1$ cm$^{-3}$ on the upstream density. We note that slower $v_{\rm sh}$ would allow higher densities.
...
\be
M_{\rm swept} < n_{\rm max} m_p V_{\rm X} \sim 0.15M_{\odot} \sim 150M_{\rm J},
\ee
