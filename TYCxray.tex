...
\author[0000-0002-9632-6106]{Ken Shen}
...
\begin{abstract}
Stars in binary systems with small separations can interact through mass transfer; therfore, stars in binary or multiple systems may follow very different evolutionary pathways than single stars.
TYC 2597-735-1 is a candidate for a recent stellar merger remnant where a low-mass companion merged with the primary of the system a few thousand years ago. This violent event is evident in a conical outflow, seen brightly in H$\alpha$ and the UV. From archival Chandra data, we identify X-ray emission from TYC 2597-735-1 itself with $\log(L_\mathrm{X}/L_\mathrm{bol})=-5.3$  and from two small regions close to the shock fronts in the outflow. A TESS lightcurve confirms a period of about 14~days. Together, this indicates ongoing stellar activity on TYC 2597-735-1, which would not be expected from the star of its luminosity class. Using stellar evolution models, we interpret the stellar activity as a sign of a newly formed convection zone in the stellar atmosphere that could lead to a convective dynamo. Either clumping in the ISM or an inhomogeneous mass and velocity distribution in the outflow could explain the localized X-ray emission in the outflow.
...
Other examples include \object{BP Psc} \citep{Zuckerman_2008}, and FK~Com type stars \citep[e.g.][]{2016ApJS..223....5A}.
This makes TYC 2597-735-1 a rare astrophysical laboratory where we can test physical models of stellar merger behaviors, characteristics, and long-term evolution.
...
\cite{2020Natur.587..387H} interpret the combination of the bright BRN and the unusual stellar parameters in a stellar merger scenario, where a low-mass ($0.1\;\mathrm{M}_\sun$), short-period companion merged with the primary star a few thousand years ago. In the process of that merger, a circumstellar disk with an inner radius of 0.1~au was formed. Today, material from that disk is accreting onto the central star. The merger also caused the ejection of a conical outflow moving at velocities of around 400~km~s$^{-1}$. Our line-of-sight is close to the axis of the outflow and we see the approaching and receeding flows projected onto the sky as the rings of the BRN.
The BRN is seen brightly in the UV as a diffuse ring and, at further distance from TYC 2597-735-1, narrow H$\alpha$ filaments. \cite{2020Natur.587..387H} explain the H$\alpha$ as the forward shock of the outflow, caused by the ejecta driving a shock wave into the ISM, while the UV emission shoudl be dominated by molecular H$_2$ emission, that is excited by a reverse shock. This reverse shock is caused by the decelatation of the outflow with the ISM and now travels backwards through the outflow.
...
We extract spectra for all three sources using circular extraction regions that encompass 90\% of the PSF at 1.5~keV at the location of the source. A background spectrum is extracted from a large region on the same chip, that is located outside of the UV contours shown in Fig~\ref{fig:chandraimage}. Below 7~keV, the background is well described by a flat component, except for a significant feature around 2.2~keV \citep[caused by fluorescences of gold in the instrument][]{2021arXiv210811234S}. Therefore, we exclude the regions 2.0-2.5~keV and $>7.0$~keV from fitting. We then simultaneously fit the source and background spectra using a modified Cash statistic \citep{1979ApJ...228..939C} as implemented in Sherpa \citep{2007ASPC..376..543D,doug_burke_2021_4428938}. To fit source emission, we use a model of an absorbed, collisionally-excited plasma \citep[APEC model,][]{2012ApJ...756..128F} and add the flat background model, scaled for the different extraction regions. That way, we avoid subtracting the background and retain Poisson statistics. All fits are done on unbinned data, but the spectra are shown binned for display in Figs.~\ref{fig:TYC_spec} and \ref{fig:combined}.

The spectrum for TYC 2597-735-1 is shown in Fig.~\ref{fig:TYC_spec}. We fit $N_\mathrm{H}=(6\pm5)\times10^{21}$~cm$^{-2}$ and a plasma temperature in the range 0.1-1.5~keV, where the low end of the range requires very large values of $N_\mathrm{H}$ and emission measure. For temperatures around 1~keV, common for stellar sources, the unabsorbed flux is of order $10^{-13}$~erg~s$^{-1}$~cm$^{-2}$, which corresponds to $3\times 10^{30}$~erg~s$^{-1}$ with uncertainties of a factor a few.
...
Assuming the hotter spectrum, a weaker source ($2\times10^{-14}$~erg~s$^{-1}$~cm$^{-2}$, which corresponds to $5\times 10^{29}$~erg~s$^{-1}$) could be sufficient to explain the observed photons; on the other hand, there could still be a second, cooler, component that is sitll hidden form view by the $N_\mathrm{H}$, thus the flux given is a lower limit only. \cite{2020Natur.587..387H} found essentially no reddening towards TYC 2597-735-1, but depending on the position of the X-ray emitting gas it is possible that accretion columns or the disk are in our line-of-sight and that the $N_\mathrm{H}$ observed in X-rays is larger than the reddening of the optical spectra.

\begin{figure}
    \plotone{figures/TYC_spec.pdf}
    \caption{Spectrum of TYC 2597-735-1 binned to two counts per bin for display purposes. Error bars are shown just to guide the eye. The orange line shows the best fit model with $N_\mathrm{H}$ as a fitted parameter, for the red model $N_\mathrm{H}$ is fixed as described in the text. Black crosses show the background rate, scaled from a much larger area. The background feature that caused us to ignore the region 2.0-2.5~keV in the fit is also visible. Error bars on the background are omitted for clarity.
    \label{fig:TYC_spec}}
\end{figure}
...
Before we discuss possible mechanisms to generate X-ray for TYC 2597-735-1 in sect.~\ref{sec:discussTYC} and for the resolved outflow in sect.~\ref{sec:outflow}, we summarize the density and temperatures in strong hydrodynamical shocks, since several possible scenarios involed shock heated plasma.
...
%\end{equation}
%or
$v_{\rm ff} \lesssim$ 260 km/s. At this free fall velocity, the temperature of the shocked material according to eqn~\ref{eqn:Tshock} is $\lesssim8\times 10^5$~K -- too low to explained the observed X-ray flux.
...
If we assume that a fraction $\eta_{\rm j} \sim 10\%$ of the accreted matter is launched into a bipolar collimated outflow (``jet'') with an outflow velocity $v_{\rm j}$ some multiple $\sim 3$ of the surface escape speed of TYC, $\approx (2GM_{\star}/R_{\star})^{1/2} \approx 230$ km s$^{-1}$, then the present-day jet power is given by
\be
...
The Rossby number, which is the ratio of convective overturn time to rotational period, i.e.
\begin{equation}
...
\citet{2020Natur.587..387H} used a series of \texttt{MESA} models to simultaneously track the temporal behavior of a recent stellar merger to match its properties to observed characteristics of TYC 2597-735-1 and estimate the original binary star properties (in particular, M$_{\star}$) that best matches the anticipated age (from the expansion rate of the BRN) and observed properties of TYC 2597-735-1 today. Using the best-fit model output from \citet{???}\textcolor{blue}{HMG:This reference somehow got messed up. Does anyone know what reference should be here?}, we extract the properties of possible convection zones that may have arisen in TYC 2597-735-1 post-merger with a 0.1 M$_{\odot}$ companion. The primary star in the \texttt{MESA} evolutionary model has a mass M$_{\star}$ = 2.17 M$_{\odot}$ and a radius R$_{\star}$ = 5 R$_{\odot}$. The models suggest the primary (surviving) star may have two prominent convection zones that persist $>$5,000 years after the stellar merger: a deep convection zone ($\sim$0.5 R$_{\odot}$), and a surface one. As the dynamo near the stellar surface is the one that likely controls magnetic activity, we focus on the properties of the surface convection zone of our modeled star.
% Table here
\input mesatable.tex
...
We estimate the Rossby number (${\rm Ro}$) at each timestamp in our model by assuming that angular momentum at the stellar surface is conserved over time. The merger itself inflates the stellar radius, which means the rotation period at earlier times would have to be longer than the 13.75 day rotation observed by \citet{2020Natur.587..387H} and TESS (Section 3.4) to conserve angular momentum. Even then, Ro is very large ($>$20) until t$\lesssim$2,000 years.  \citet{Pizzolato+2003} show that magnetic activity in main sequence stars ceases for ${\rm Ro} \gtrsim 10$, suggesting that for much of the time close to the merging of the two stars, TYC 2597-735-1 may not have significant magnetic activity.
At later times (t$\gtrsim$2,000 years), ${\rm Ro}$ is within the regime where magnetic activity is present.

The predicted X-ray luminosity from the dynamo is (e.g., \citealt{Soker&Tylenda2007})
\begin{equation}
\frac{L_{\rm X}}{L_{\rm bol}} = C_{\rm x}{\rm Ro}^{-2},
\end{equation}
where the constant $C_{\rm x}$ varies with the stellar system. We present $C_{\rm x}$ for each timestamp in the \texttt{MESA} model in Table~\ref{tab:mesa}. \citet{Soker&Tylenda2007} note that $C_{\rm x}$ is valid for 0.15 $\lesssim$ ${\rm Ro} \lesssim$ 10, where $L_{\rm X} / L_{\rm bol}$ saturates at values $\sim$10$^{-3}$ (${\rm Ro} \lesssim$ 0.15). In the 2,000-5,000~yr range of our models, $C_{\rm x}$ ranges from 10$^{-3}$ - 5$\times$10$^{-5}$. The lower end is in line with typical main sequence-to-early subgiant magnetic activity levels ($C_{\rm x} \sim$ 10$^{-5}$-10$^{-6}$; \citealt{Pizzolato+2003, Gondoin+2005}).
...
Combining $C_{\rm x}$ and Ro, we find $\log(L_\mathrm{X}/L_\mathrm{bol})=-5.1$ for the 2,000-5,000~yr range of our models with is fully compatible with the observed value of -5.3, given the uncertainties in the X-ray flux discussed in Section~\ref{sec:xrayspectra}.
...
X-ray data and luminosities for central objects of PN are taken from \citet{2010ApJ...721.1820M}. The central objects of \object{HFG 1} and \object{DS 1} are members of close binary systems, we show them in Figure~\ref{fig:lxbol} assuming that their rotation period matches their orbital period. Both objects are rapid rotators and and show other signs of magnetic activity as well; DS~1 may have undergone a flare during the observation, which might explain why its observed $\log(L_\mathrm{X}/L_\mathrm{bol})$ is brighter than the saturation limit \citep{2010ApJ...721.1820M}. The central object of \object{LoTr 5} has been repeatedly observed with Doppler-imaging \citep{2019A&A...624A..83K}, so the period is well known. The objects displays both cool and warm spots. Warm spots live longer (three months) and are located on average at higher latitudes than cool spots; the surface flow has an anti-solar differential rotation.

Figure~\ref{fig:lxbol} shows that all those different classes of objects behave remarkably similar: Despite some scatter, stars of different sizes and with different evolutionary histories show a clear increase in $\log(L_\mathrm{X}/L_\mathrm{bol})$ with decreasing period. The (single or binary) field giants tend to be a little to the right (in the left panel) of the MS stars, i.e.\ they reach a similar level of $\log(L_\mathrm{X}/L_\mathrm{bol})$ for a longer rotation period than MS stars; on the other hand, they are to the left of MS stars in Rossby number (right panel). Our recent merger candidates (FK Com, TYC 2597-735-1, and BP Psc) as well as the central stars in planetary nebula behave in the opposite way: They tend to have shorter periods than MS stars for a given $\log(L-\mathrm{X}/L_\mathrm{bol})$, but --at least in the case of TYC 2597-735-1 where we have stellar structure simulations to estimate the Rossby number-- a larger Rossby number for a given $\log(L_\mathrm{X}/L_\mathrm{bol})$.

{\color{blue}What is different between field giants and those that are spun up with mass transfer?}

{\color{blue} PCS: Could the differences in position (left/right of MS stars) in the Prot/Ro plots be a result of uncertainties/errors in Ro. In the end, Prot is observed while Ro is calculated/estimated... In other words, can we use this to imrpove on the Ro esimate?}

\subsection{Large scale outflow}
\label{sec:outflow}
We now look at what limits on density and mass flux we can place from the detection of X-East and X-North (Sect.~\ref{sec:xEN}) and the upper limit on X-ray emission from the large region between forward and reverse shock (Sect.~\ref{sec:ISM}.
...
Assume the BRN ejecta of density $n_{\rm BRN}$ is expanding at $v_{\rm ej} \simeq 400$ km/s into a stationary external medium of particle density $n_{\rm ism}$. Given the bright UV emission, we expect that the shock has already swept up considerable gas from the external medium and begun to decelerate, then $v_{\rm fsh} \ll v_{\rm ej}$ and $v_{\rm rsh} \simeq -v_{\rm ej}/3$, using eqns~\ref{eqn:vsh1} and \ref{eqn:vsh2}.
...
\ee
however depending sensitively on $kT_{\rm sh}$ (and hence $v_{\rm sh}$).  Thus, given a limit %$L_{\rm E_X > 1 keV}  < 6\times 10^{30}$ erg s$^{-1}$,
of few times $10^{31}$~erg~s$^{-1}$~pc$^{-2}$ (section~\ref{sec:xraydetection})
we can put an upper limit $n < 1$ cm$^{-3}$ on the upstream density.
...
\be
M_{\rm swept} \sim n m_p V_{\rm X} \sim 0.15M_{\odot} \sim 150M_{\rm J},
\ee
...
\label{sec:summary}
We find X-ray emission from TYC 2597-735-1 itself and from two small regions in the outflow. The emission in all three regions is concentrated between about 1 and 2~keV, consistent with a soft, lightly absorbed plasma. We can place an upper limit on large scale emission from the entire reverse-shock region. For TYC 2597-735-1 itself, we also show TESS lightcurve. Despite stray light, it is clear that the star itself is photometrically variable and this variability appears consistent with a period of about 14~days, seen before in RV. X-rays and optical variability can both be explained by coronal activity, which implies the presence of a convective dynamo. Such a dynamo is, in fact, predicted in evolutionary models and the modeled $\log(L_\mathrm{X}/L_\mathrm{bol})=-5.1$ is consistent with the observed $\log(L_\mathrm{X}/L_\mathrm{bol})=-5.3$ given the systemtic uncertainties in the spectral fitting.
We compare the activity in TYC 2597-735-1 with pre-main sequence and main-sequence dawrfs, with single and binary giants, and with other stellar merger candidates. In general TYC 2597-735-1 shows a similar behaviour as other moderately active stars, with an indication that its rotation period is slight shorther and its Rossby number slightly larger than for other stars of comparable $L_\mathrm{X}/L_\mathrm{bol}$.

We also tentatively detect two X-ray sources in the outflow, located between the forward and the reverse shock. These might be caused by inhomogeneties of the outflow that lead to signficantly higher densities in these regions, compared to the general upper limit of $n < 1$ cm$^{-3}$ on the average upstream density.

The Chandra data presented here was taken serendiputously with an exposure time less than 10~ks. A deeper observation with Chandra or XMM-Newton can confirm the detection of the X-North and X-East regions and test the hypothesis that regions in the outflow might stocastically become brighter and fade away again on time scales of 20~years; new observations will also reduce the error bars on the spectral properties of TYC 2597-735-1 itself. If the spectrum can be resolved into a low and high-temperature component, a magnetic corona is required, while the absence of a hotter component indicates an outflow shock. Future instruments with much higher effective area and resolving power are needed to definitively resolve any ambiguity between absorption and soft plasma.
